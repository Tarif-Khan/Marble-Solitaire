...
/**
 * This is the factory class for an english model view where it makes the model and view
 * with a given list of integers from the main method.
 */
public class EnglishMVFactory implements Factory {
   ...
   */
  public EnglishMVFactory(List<Integer> inputList) {
    if (inputList.size() == 0) {
      this.model = new EnglishSolitaireModel();
    }
    if (inputList.size() == 1) {
      this.model = new EnglishSolitaireModel(inputList.get(0));
    }
    if (inputList.size() == 2) {
      this.model = new EnglishSolitaireModel(inputList.get(0), inputList.get(1));
    }
    if (inputList.size() == 3) {
      this.model = new EnglishSolitaireModel(inputList.get(0), inputList.get(1), inputList.get(2));
    }
  }
